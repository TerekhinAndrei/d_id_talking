...
export const useMicrophoneToDid = (options = {}) => {
  // Состояния
  ...
  // 🎵 ОЧЕРЕДЬ ЗАГРУЗКИ В D-ID - НОВАЯ АРХИТЕКТУРА
  const uploadQueueRef = useRef([]);
  const isUploadingRef = useRef(false);

  // Обработчик очереди загрузки в D-ID
  const processUploadQueue = useCallback(async () => {
    ...
    while (uploadQueueRef.current.length > 0) {
      ...
      try {
        console.log('🎯 Загружаем аудио в D-ID из очереди, осталось:', uploadQueueRef.current.length);
        ...
        // Загружаем через новую систему хранения файлов в D-ID
        const uploadOptions = new UploadOptions({
          provider: StorageProvider.D_ID,
          strategy: UploadStrategy.FALLBACK, // Fallback на Cloudinary если D-ID недоступен
          onProgress: (progress) => console.log('Upload progress:', progress),
          onSuccess: (result) => console.log('✅ Audio uploaded successfully to D-ID:', result),
          onError: (error) => console.error('❌ Audio upload failed:', error)
        });
        
        const result = await fileStorageService.uploadAudio(audioFile, uploadOptions);
        
        if (result.success) {
          const didUrl = result.fileMetadata.url;
          console.log('✅ Аудио загружено в D-ID:', didUrl);
          ...
          try {
            if (options && typeof options.onAudioUploaded === 'function') {
              options.onAudioUploaded(didUrl, result.provider);
            }
          } catch (error) {
          ...
          throw new Error(result.error || 'Ошибка загрузки в D-ID');
        }
        
      } catch (error) {
        console.error('❌ Ошибка загрузки аудио в D-ID из очереди:', error);
  ...
  const addToUploadQueue = useCallback((audioData) => {
    ...
    console.log('📥 Добавлен в очередь загрузки D-ID, размер очереди:', uploadQueueRef.current.length);
  ...
  const connectWebSocket = useCallback(() => {
    try {
      ...
      websocketRef.current.onopen = () => {
        console.log('✅ WebSocket соединение установлено для D-ID загрузки');
      };
      
      websocketRef.current.onmessage = async (event) => {
        try {
          const message = JSON.parse(event.data);
          console.log('📨 Получено WebSocket сообщение для D-ID:', message);
          
          if (message.type === 'audio_data') {
            // 🎯 ИСПОЛЬЗУЕМ ОЧЕРЕДЬ ЗАГРУЗКИ В D-ID ВМЕСТО CLOUDINARY
  ...
  const loadAudioWorklet = useCallback(async () => {
    ...
    try {
      ...
      console.log('✅ AudioWorklet загружен успешно для D-ID');
  ...
  const createAudioWorkletNode = useCallback(async (stream) => {
    try {
      ...
      audioWorkletNodeRef.current.port.onmessage = (event) => {
        ...
        switch (type) {
          ...
          case 'send_phrase':
            ...
            console.log('📤 Отправляем фразу через WebSocket для D-ID');
            sendPhrase([data]);
            break;
      ...
      console.log('🎵 AudioWorklet узел создан и запущен для D-ID');
  ...
  const sendAudioChunk = useCallback((audioData) => {
    ...
    try {
      ...
      console.log('📤 Отправляем WebSocket сообщение для D-ID:', message);
      websocketRef.current.send(JSON.stringify(message));
      console.log('📤 Аудио чанк отправлен через WebSocket для D-ID');
  ...
  const startRecording = useCallback(async (voiceId) => {
    try {
      ...
      setIsRecording(true);
      console.log('🎤 Запись начата с AudioWorklet для D-ID загрузки');
  ...
  const stopRecording = useCallback(() => {
    try {
      ...
      console.log('⏹️ Запись остановлена, AudioWorklet отключен, очередь D-ID очищена');
  ...
  const sendPhrase = useCallback(async (audioBuffer) => {
    ...
    try {
      ...
      console.log('📤 Отправляем фразу для D-ID:', {
  ...
  const processAudioChunk = useCallback((audioData, hasSpeech) => {
    ...
    if (hasSpeech && websocketRef.current && websocketRef.current.readyState === WebSocket.OPEN) {
      console.log('🎤 Отправляем аудио чанк с речью для D-ID');
