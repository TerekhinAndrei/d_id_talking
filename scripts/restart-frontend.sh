...
# 🔄 Скрипт перезапуска фронтенда Talking Head
# Автор: AI Assistant
...
# Основная функция
main() {
    echo -e "${BLUE}🔄 Перезапуск фронтенда Talking Head${NC}"
    echo "=================================="
    ...
    if ./scripts/start-frontend.sh; then
        success "Фронтенд успешно перезапущен!"
    else
        error "Не удалось перезапустить фронтенд"
        exit 1
    fi
}
