...
# 🛑 Скрипт остановки фронтенда Talking Head
# Автор: AI Assistant
...
# Остановка фронтенда по порту
stop_by_port() {
    local pids=$(lsof -ti :$FRONTEND_PORT 2>/dev/null)
    
    if [ -n "$pids" ]; then
        log "Найдены процессы на порту $FRONTEND_PORT: $pids"
        ...
        if ! lsof -ti :$FRONTEND_PORT >/dev/null 2>&1; then
            success "Все процессы на порту $FRONTEND_PORT остановлены"
            return 0
        else
            error "Не удалось остановить все процессы на порту $FRONTEND_PORT"
            return 1
        fi
    else
        log "Процессы на порту $FRONTEND_PORT не найдены"
        return 0
    fi
}
...
# Основная функция
main() {
    echo -e "${BLUE}🛑 Остановка фронтенда Talking Head${NC}"
    echo "=================================="
    ...
    if [ "$stopped" = true ]; then
        success "Фронтенд успешно остановлен!"
        log "Для запуска используйте: ./scripts/start-frontend.sh"
    else
